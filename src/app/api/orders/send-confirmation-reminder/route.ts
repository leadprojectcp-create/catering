...
async function sendFcmNotification(
  ...
  try {
    ...
    if (!userDoc.exists) {
      console.log('[구매확정 알림 FCM] 사용자 문서 없음:', userId)
    ...
    if (!fcmToken) {
      console.log('[구매확정 알림 FCM] FCM 토큰 없음:', userId)
    ...
    console.log('[구매확정 알림 FCM] 전송 성공, Message ID:', response)
    return { success: true, messageId: response }

  } catch (error) {
    const errorMessage = (error as Error)?.message || ''
    console.error('[구매확정 알림 FCM] 전송 실패:', errorMessage)
    ...
    if (
      ...
      try {
        ...
        console.log('[구매확정 알림 FCM] 유효하지 않은 토큰 삭제:', userId)
      } catch (fsError) {
        console.error('[구매확정 알림 FCM] 토큰 삭제 실패:', fsError)
      }
    }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const fcmResult = await sendFcmNotification(
      ...
      orderData.uid,
      '구매확정을 해주세요!',
      `${orderData.storeName}에서 주문하신 상품이 배송완료 되었습니다. 구매확정을 눌러주세요.`,
      {
