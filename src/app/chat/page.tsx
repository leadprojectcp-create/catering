...
export default function ChatPage() {
  ...
  const [selectedRoomPhone, setSelectedRoomPhone] = useState<string>('')

  // URL에 roomId가 있으면 채팅방 정보를 불러와서 이름과 전화번호 설정
  useEffect(() => {
    if (!roomId || !user) {
      setSelectedRoomName('')
          ...
          if (otherUserId) {
            ...
            if (otherUserDoc.exists()) {
              ...
              if (otherUserData.phone) {
                setSelectedRoomPhone(otherUserData.phone)
              } else {
                setSelectedRoomPhone('')
              }
            }
  ...
  return (
    <>
      {isPartner ? (
        <PartnerHeader
          chatRoomTitle={selectedRoomName || undefined}
          chatRoomPhone={selectedRoomPhone || undefined}
        />
      ) : (
        <Header
          chatRoomTitle={selectedRoomName || undefined}
