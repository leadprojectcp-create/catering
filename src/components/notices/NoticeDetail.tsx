...
export default function NoticeDetail({ noticeId }: NoticeDetailProps) {
  ...
  const loadNotice = async () => {
    try {
      ...
      // 이전글/다음글 찾기
      const allNotices = await getPublishedNotices('user')
      ...
      <div className={styles.content}>
        ...
        <div className={styles.footer}>
          <button
            ...
            onClick={() => prevNoticeId && router.push(`/notices/${prevNoticeId}`)}
          ...
          <button
            className={styles.navButton}
            onClick={() => nextNoticeId && router.push(`/notices/${nextNoticeId}`)}
